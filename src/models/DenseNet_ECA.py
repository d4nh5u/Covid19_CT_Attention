...
class DenseLayer(nn.Sequential):
  ...
  def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
    ...
    # Attention
    self.eca = ECABlock(num_input_features + growth_rate)
  ...
  def forward(self, x):
    ...
    # Attention
    concate_output = torch.cat([x, output], 1)
    concate_output = self.eca(concate_output)
...
class DenseNet(nn.Module):
  def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=224, bn_size=4, compression_rate=0.5, drop_rate=0, num_classes=2):
    ...
    if block_config == (6, 12, 24, 16):
      self.name = "DenseNet121_ECA"
    elif block_config == (6, 12, 32, 32):
      self.name = "DenseNet169_ECA"
...
if __name__ == '__main__':
    ...
    model = DenseNet121_ECA()
    model.to('cpu')
    summary(model, input_size=(1, 3, 224, 224), depth=1)
    
    model_1 = DenseNet169_ECA()
    model_1.to('cpu')
    summary(model_1, input_size=(1, 3, 224, 224), depth=1)